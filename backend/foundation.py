...
def _norm(text: str) -> str:
    # 只比字，不比标点：空白、强调符号和标点都不算改动。
    # 这样模型能在原地把「会照着写、但不知道」改成「会照着写，但不知道」，不用划掉重写。
    # 标点也算改动的话，写错的顿号会被「原文照抄」锁住，散文还会照着清单抄。
...
def describe_for_model(problems: list[str]) -> str:
    ...
    return (
        "上一次输出没有通过地基清单的校验：\n"
        f"{lines}\n\n"
        "清单的纪律是：上一版的每条都保留编号；生效的条目原文照抄，只有标点可以改；要改一条就把它划掉，"
